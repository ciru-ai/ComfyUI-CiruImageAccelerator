...
def dense_attention(
    ...
    mx = tl.full((BM,), float("-inf"), tl.float32)
    den = tl.full((BM,), 0.0, tl.float32)
    out = tl.full((BM, D), 0.0, tl.float32)
    for start in range(tl.cdiv(N, BN)):
        ...
        nxt = tl.maximum(mx, tl.max(score, axis=1))
        p = tl.exp2(score - nxt[:, None])
        factor = tl.exp2(mx - nxt)
        v = tl.load(V + h * SVH + ns[:, None] * SVM + ds[None, :], ns[:, None] < N, 0)
        out = out * factor[:, None]
        out = tl.dot(p.to(q.dtype), v, out)
        den = den * factor + tl.sum(p, axis=1)
        mx = nxt
    tl.store(
        O + h * M * D + rows[:, None] * D + ds[None, :],
        (out / den[:, None]).to(O.dtype.element_ty),
        rows[:, None] < M,
    )
...
def attention(q, k, v, bm=64, bn=64, warps=4):
    ...
    dense_attention[(triton.cdiv(m, bm), h)](
        q,
        k,
        v,
        out,
