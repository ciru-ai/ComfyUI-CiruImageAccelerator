...
Image editing uses the **same Ciru AMD Halo Qwen 2.1 Turbo node**, not a separate edit node. Open **Templates → Extensions → ComfyUI-CiruImageAccelerator → qwen21-1024-image-edit-full**, or load [`workflows/qwen21-1024-image-edit-full.gui.json`](workflows/qwen21-1024-image-edit-full.gui.json) into ComfyUI. In its **Load Image** node, choose the source picture; the bundled [`assets/elephant-1024-full.png`](assets/elephant-1024-full.png) is an optional example you can upload. Write the edit instruction in `TextEncodeQwenImage21` and refer to the source as `<image1>`. The template connects the image and VAE to that encoder, its latent output to `KSampler`, and the Ciru node on the `MODEL` connection between `QwenImage21Cache` and `KSampler`.

The edit template starts at 30 sampler steps and 30 `full_evaluations`, which performs every denoiser evaluation. `auto` keeps native attention at 1024 and uses packed attention for the qualified 2048-square shape on gfx1151. To try faster edit prediction, explicitly enable `allow_edit_prediction` and lower `full_evaluations`; this is experimental, so compare with a full-evaluation edit before relying on source fidelity. If you use ComfyUI's official **Image Edit (Qwen Image 2.1)** workflow instead, insert the Ciru node on its `MODEL` connection after `QwenImage21Cache` and before the sampler.

For text-to-image, open **Templates → Extensions → ComfyUI-CiruImageAccelerator** and choose the 1024 or 2048 GUI workflow. The editable node controls are visible between the cache and sampler. The same text-to-image setups are in `api_examples/` for API users; use `qwen21-1024-default.api.json` or `qwen21-2048-default.api.json`, and the `-full` files for 30/30 references. The GUI workflows in `workflows/` are what ComfyUI shows in its Templates menu.
